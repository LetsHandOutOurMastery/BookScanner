...
class SqliteXlsConverter {
    ...
    fun exportSqliteToExcel(context: Context) {
        ...
        val fileName = "BooksCollection_${CommonUtils.getNowDateForFileName()}.xls"
        val file: File = File(folderPath, fileName)
        file.createNewFile()

        //mXlsFilePath = file?.path
        //export excel file from sqlite file
        val sqliteToExcel = SQLiteToExcel(context, Constants.DATABASE_NAME, downloadFolder.toString())
        sqliteToExcel.exportAllTables(fileName, object : SQLiteToExcel.ExportListener {
            ...
            override fun onCompleted(filePath: String) {
                sqliteToXlsExportListener?.onExportComplete(filePath, downloadFolder.toString(), fileName)
